...
const EnrollmentModal: React.FC<{
  ...
  const [formData, setFormData] = useState<EnrollmentFormData & { selectedProgramSlug: string }>({
    fullName: '',
    email: '',
    phone: '',
    institution: '',
  ...
  const validate = (): boolean => {
    ...
    if (!program && !formData.selectedProgramSlug) {
      nextErrors.form = 'Please select a training program.';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const programTitle =
      program?.title || TRAINING_PROGRAMS[formData.selectedProgramSlug]?.title || 'Unknown Program';
        ...
        {isOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-black border-[4px] sm:border-[6px] border-[#00BFFF] shadow-[20px_20px_0px_0px_rgba(0,191,255,0.15)] overflow-hidden flex flex-col"
            >
              <div className="p-4 sm:p-6">
                <button
                  onClick={onClose}
                  className="absolute top-4 right-4 text-white/40 hover:text-[#00BFFF] transition-colors z-10"
                >
                  <X size={20} />
                </button>

                <div className="relative mb-4 sm:mb-6">
                  <span className="text-[9px] font-black text-[#00BFFF] uppercase tracking-[0.4em] mb-1 block">
                    Enrollment Portal
                  </span>
                  <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter leading-none">
                    {program ? `Join ${program.title}` : 'Enroll in a Program'}
                  </h2>
                  <div className="absolute -left-10 top-1/2 -translate-y-1/2 w-1 h-10 bg-[#00BFFF]" />
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={handleInputChange}
                        className="w-full bg-white/5 border-[2px] border-white/10 p-1.5 sm:p-2 text-white font-bold text-[8px] sm:text-[10px] focus:border-[#00BFFF] outline-none appearance-none cursor-pointer uppercase tracking-tight"
                      >
                      ...
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[#00BFFF]">
                        <ChevronDown size={14} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-2 sm:gap-3">
                    <div className="space-y-1 group">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className={cn(
                          'w-full bg-white/5 border-[2px] border-white/10 p-1.5 sm:p-2 text-white font-bold text-[8px] sm:text-[10px] focus:border-[#00BFFF] outline-none transition-all placeholder:text-white/20',
                  ...
                  <button
                    ...
                    className="w-full py-2 sm:py-3 bg-[#00BFFF] border-[2px] border-black text-black font-black uppercase tracking-[0.2em] text-[8px] sm:text-[9px] flex items-center justify-center gap-2 hover:bg-white transition-all active:scale-95 disabled:opacity-50"
                  >
                    {isSubmitting ? (
                      <>
...
}> = ({ isOpen, onClose, program, onEnroll }) => {
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6 lg:p-8">
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/10 shrink-0">
              <div>
                <span className="text-[9px] font-black text-[#00BFFF] uppercase tracking-[0.3em] mb-1 block">
                  {program.badge}
                </span>
                ...
                <div className="lg:col-span-7 space-y-8">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-semibold text-[#00BFFF] uppercase tracking-widest">
                        What You Will Learn
                      </h4>
                      <ul className="space-y-2">
                        {program.skills?.technical?.slice(0, 4).map((skill: string, i: number) => (
                          <li
                            key={i}
                            className="flex items-start gap-2 text-[12px] text-white/70 font-normal leading-snug"
                          >
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="bg-black/40 p-5 border border-white/5 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-semibold text-[#00BFFF] text-right max-w-[150px]">
                        {program.audience}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <Link
                      ...
                      className="w-full py-4 border-2 border-[#00BFFF] hover:bg-[#00BFFF] text-white hover:text-black text-[11px] font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all active:scale-95"
                    >
...
const ProgramCard: React.FC<{
  ...
  openEnroll: (slug: string) => void;
}> = ({ item, progData, openOverview, openEnroll }) => (
  <div
  ...
  >
    ...
    <div className="relative h-32 sm:h-40 overflow-hidden border-b-2 border-black">
      ...
      <div className="absolute bottom-3 left-3 px-3 py-1 bg-black/80 backdrop-blur-sm border border-[#00BFFF]/30">
        <span className="text-[9px] font-black text-[#00BFFF] uppercase tracking-widest">
          {progData?.badge || 'Training Program'}
        </span>
      </div>
    </div>
    ...
    <div className="p-4 sm:p-6 flex flex-col flex-1">
      <div className="flex items-start justify-between gap-4 mb-4">
        <h4 className="text-sm sm:text-base lg:text-lg font-black text-white uppercase tracking-tighter leading-tight group-hover:text-[#00BFFF] transition-colors">
          {item.label}
        </h4>
      </div>
      ...
      <p className="text-[11px] sm:text-xs text-white/60 font-normal leading-[1.75] mb-6 flex-1">
        {progData?.description ||
          'Institutional capacity building and professional skill enhancement.'}
      </p>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <div className="flex flex-col">
          ...
          <span className="text-[9px] sm:text-[10px] font-black text-[#00BFFF] uppercase">
            {progData?.duration || 'Variable'}
          </span>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => openEnroll(item.slug)}
            className="px-3 h-7 sm:h-8 flex items-center justify-center border border-[#00BFFF]/30 hover:border-[#00BFFF] text-[#00BFFF] hover:bg-[#00BFFF] hover:text-black text-[9px] font-black uppercase tracking-widest transition-all bg-black/40"
            title="Enroll Now"
          >
            Enroll
          </button>
...
const ProgramCarousel: React.FC<{
  ...
  const btnActive =
    'border-white/20 bg-black/40 text-white hover:bg-[#00BFFF] hover:text-black cursor-pointer';
  const btnDisabled =
    'border-white/5 bg-black/10 text-white/15 cursor-not-allowed pointer-events-none';
...
const Training: React.FC = () => {
  ...
  const handleEnrollFromOverview = (program: TrainingProgram) => {
    setSelectedProgram(program);
  ...
  return (
    <div className="relative min-h-screen bg-black overflow-hidden">
      ...
      <div className="relative z-30 bg-black border-b border-white/5 px-4 sm:px-6 py-2.5 flex items-center gap-3">
        ...
        >
          <Home size={14} className="text-[#00BFFF]" />
          <span>Home</span>
        </Link>
                  ...
                  <div className="select-none inline-block bg-black py-3 px-6 sm:px-8 border-4 border-black">
                    <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black font-heading text-white uppercase tracking-tighter">
                      {cat.title.split(' ').length > 1 ? (
                        <>
                          {cat.title.substring(0, cat.title.lastIndexOf(' '))}{' '}
                          <span className="text-[#00BFFF]">
                            {cat.title.substring(cat.title.lastIndexOf(' ') + 1)}
                          </span>
                        </>
                      ) : (
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,#00bfff_50%,#000000_50%)] flex flex-col items-center justify-center py-10 sm:py-14 px-4">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 w-full flex flex-col items-center">
          ...
          <div className="relative w-full max-w-4xl flex items-center justify-center lg:justify-start">
            <div className="absolute right-[-10%] lg:right-[-15%] top-1/2 -translate-y-1/2 w-[300px] sm:w-[400px] lg:w-[500px] aspect-[4/5] z-[-1] opacity-100">
              <img
            ...
            >
              <div className="relative z-10 text-center sm:text-left">
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-white leading-tight mb-4 uppercase tracking-tighter">
                  Ready to <span className="text-[#00BFFF]">Upskill</span> Your
                  <br className="hidden sm:block" /> Team or Organization?
                </h2>
                <p className="text-white/80 text-xs sm:text-sm mb-8 leading-relaxed font-normal max-w-lg">
                  Whether you&apos;re looking to enroll individually, run a corporate training
                  program, or design a custom curriculum — our team is ready to work with you.
                </p>
                <div className="flex flex-col sm:flex-row items-center sm:items-start justify-center sm:justify-start gap-3 mb-10">
                  ...
                  >
                    Enroll Individually <ArrowRight size={14} />
                  </button>
                  <Link
                    to="/contact?service=Corporate Training"
                    className="h-11 sm:h-12 px-6 border-2 border-[#00BFFF] text-[#00BFFF] rounded-none font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-[#00BFFF] hover:text-black transition-all transform hover:-translate-y-1 active:scale-95 flex items-center gap-2"
                  >
