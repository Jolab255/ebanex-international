...
const TopBar: React.FC = () => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {activePanel === 'phone' && (
          <motion.div
            ...
            className="fixed top-1/2 -translate-y-1/2 right-0 z-50 bg-black/95 backdrop-blur-md border-l border-[#00BFFF]/20 shadow-2xl w-full max-w-[320px] sm:max-w-[280px]"
          >
            <div className="p-3 sm:p-4 flex items-center gap-3">
              <div className="flex items-center justify-center w-10 h-10 bg-[#00BFFF] rounded-none flex-shrink-0">
      ...
      {/* Side Icons */}
      <div className="fixed right-2 sm:right-4 top-1/2 -translate-y-1/2 z-[60] flex flex-col gap-2 hidden sm:flex">
        {contactInfo.map((item) => (
          <motion.button
            key={item.id}
            onClick={() => togglePanel(item.id)}
            className={`flex items-center justify-center w-8 h-8 rounded-none shadow-none transition-all duration-200 ${
              activePanel === item.id
                ? 'bg-[#00BFFF] text-black'
                : 'bg-black text-white hover:text-[#00BFFF] border border-white/10'
            }`}
