    ...
    <nav className={`fixed w-full z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 sm:gap-3 group">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-lg flex items-center justify-center group-hover:rotate-12 transition-transform duration-300">
        ...
        <div className="hidden lg:flex items-center gap-8">
          ...
          <Link
            to="/contact"
            className="px-4 sm:px-6 py-2 sm:py-2.5 bg-white text-slate-950 rounded-full text-xs sm:text-sm font-bold hover:bg-purple-500 hover:text-white transition-all duration-300"
          >
            Get Started
          </Link>
        </div>
        ...
        <div className="flex flex-col items-center justify-center h-full gap-8">
          {NAVIGATION_LINKS.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="text-xl sm:text-2xl font-heading font-semibold text-white hover:text-purple-400"
            >
              {link.label}
            </Link>
          ))}
